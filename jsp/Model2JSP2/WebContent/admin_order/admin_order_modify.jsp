...
<body>
	...
	<%
		...
		List adminOrderDetailList = (List)request.getAttribute("adminOrderDetail");
		
		OrderDTO orderInfo = (OrderDTO)adminOrderDetailList.get(0);
	%>
	<table border="1">
		<tr>
			<td colspan="6"><h3>구매내역</h3></td>
		</tr>
		<tr>
			<td>상품명</td>
			<td>구매수량</td>
		...
		<%for(int i=0; i<adminOrderDetailList.size(); i++){
			OrderDTO orDTO = (OrderDTO)adminOrderDetailList.get(i);
		%>
		...
		<tr>
			<td colspan="6">
				...
				배송지주소 : <input type="text" name="o_receive_addr1" value="<%=orderInfo.getO_receive_addr1()%>"><br>
				배송지 상세주소 : <input type="text" name="o_receive_addr2" <%=orderInfo.getO_receive_addr2()%>><hr>
				기타 요구사항 : <input type="text" name="o_receive_msg" value="<%=orderInfo.getO_receive_msg() %>">
				</td>
		</tr>
		...
		<tr>
			<td colspan="6">
			  ...
			  <input type="radio" name="o_trade_type" value="온라인 입금"> 온라인입금
			  <input type="radio" name="o_trade_type" value="신용카드"> 신용카드
			  <input type="radio" name="o_trade_type" value="카카오페이"> 카카오페이
			  <input type="radio" name="o_trade_type" value="삼성페이"> 삼성페이
			  <input type="radio" name="o_trade_type" value="NPay"> NPay
			</td>
		</tr>
		...
		<tr>
			<td colspan="6">
				...
				운송장번호 : <input type="text" name="trans_num">
			</td>
		</tr>
