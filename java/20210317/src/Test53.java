...
/*
6. 객체 지향 프로그래밍을 하기 위한 3단계 기법을 작성 할수 있다.
1단계 - 현실세계에 존재하는 객체를 모델링 하는 단계
2단계 - 자바 프로그램에서의 클래스class 설계도 만들기 단계(변수 ,메소드 작성)
3단계 - 자바 프로그램 측면에서의 클래스설계도를 참조 하여 객체 생성 하는 단계


사과나무 객체 

1단계  현실세계에 존재하는 객체를 모델링(현실 세계의 객체의 데이터 , 동작(기능) 추출)
...
class Tree{//나무설계도(나무 class)
	...
	//메소드만들기 :  동작(기능) - 흔들린다, 사과가 떨어지다 표현 용도
	void move() {
...
public class Test53 {
	...
	//"흔들린다" 동작을 시키기위해.. move()메소드 호출
		 tree2.move();
	//"과일이 떨어지다" 동작을 시키기위해..	  dowon()메소드 호출
		 tree2.down();
