...
public class Test70 {
	public static void main(String[] args) {
		...
		// 결론
		// static 으로 선언된 클래스 변수에 접근하는 방법
		// 1. 클래스명.클래스변수명    접근가능
		// 2. 객체 생성 후 -> 참조변수.클래스변수명   접근가능
		// ===========================================
		
		
		// Test 클래스로부터 객체 생성, 참조변수명 t2
		Test t2 = new Test(10);
		System.out.println("Test t2객체");
		
		// 인스턴스 변수 x에 접근하는 방법 : 참조변수명.인스턴스변수명
		System.out.println(t2.x);  // 10
		
		// 클래스 변수 y에 접근하는방법 : 참조변수명.클래스변수명
		System.out.println(t2.y);  // 20
		
		
		// 결론 
		// 위 출력결과를 보면, static으로 선언된 클래스변수 y는?
		// 하나의 클래스 단위당 한개의 클래스 변수만? JVM 메모리에 올라가므로
		// 위의 t1, t2 참조변수에 저장된 각 객체가
		// 클래스변수 y하나를 공유받아 사용하므로,
		// 기본에 있던 클래스변수 y내부에 저장된 값이 += 누적된다고 보면됨
		
		
		
		// 한번 더 확인!
		Test t3 = new Test(10);
		System.out.println("t3 객체");
		// 인스턴수변수 x에 접근하여 인스턴수변수 x의 값 출력
		System.out.println(t3.x);
		// 클래스변수 y에 접근하여 클래스변수 y의 값 출력
		// 방법2. 클래스명.클래스변수명;
		System.out.println(Test.y);  // 방법2 
		...
		/*
			중요결론:
			1. 객체 단위로 메모리 할당하는 인스턴스 변수는 
				인스턴스(객체) 개별적으로 따로 JVM heap영역에 
				할당하기 때문에 개별적인 인스턴스 변수로 10이 각각 저장된다
			2. 클래스변수 y는 생성된 각 객체들이 공유받아서 사용할 목적으로 
				클래스를 설계할 때 생각하고 만들어주어야 한다.
			3. 
