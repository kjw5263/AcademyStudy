...
<!--[if IE 6]>
 ...
 <script type="text/javascript">
  function winOpen() {
	  ...
	  // 중복체크 할때, 인풋에서 입력한 아이디를 팝업에 가져가는 방법은?
	  // html코드의 value값을 가져오기 
	  var userid = document.fr.id.value;
	  
	  
	  
	  // id값이 있을 때 -> 새창열기 < % =   % > 는 사용할 수 없다. java 코드가 먼저 실행되어버리기 때문이다!
	  window.open("joinIDCheck.jsp?userid="+userid,"","width=400, height=300 left=500 top=300");
  }
