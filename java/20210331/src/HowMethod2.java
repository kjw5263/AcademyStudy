...
class AreaMath1 { // 넓이 계산 클래스
	...
	// 원넓이 계산 메소드
	// main() 메소드에서도 이 클래스 선언 없이 메소드 사용하기 위해 static 선언할 것
...
class PerimeterMath1 { // 직사각형의 둘레계산클래스
	...
	public static double calRectanglePeri(double width, double height) {
		...
		// {(가로) ＋ (세로)} × 2 <--직사각형 둘레 리턴
		double result = SimpleMath1.mul(width, height) * 2;
...
public class HowMethod2 {
	...
	double circle = AreaMath1.calCircleArea(2.4);
	//참고!! 직사각형 둘레 구하는 메소드 호출, 호출시  2.0, 4.0 전달
	double nemo = PerimeterMath1.calRectanglePeri(2.0, 4.0);
		// 출력 결과
		// 원의 넓이 : 18.09504
	...
	/*
		-> static method를 선언하면, 원넓이 계산 클래스나 둘레 계산 클래스 내부에서
		SimpleMath1 객체 생성하지 않아도 된다.
