/*
	...
	제공되는 Wrapper 역할을 하는 클래스들을 미리 정의해놓고 제공함
...
public class Test124 {
	...
	public static void main(String[] args) {
		...
		// 2. Integer 래퍼 클래스의 메소드
			// int intValue() 메소드 : Integer객체에 저장된 int 값을 반환하는 메소드
			int n01 = num01.intValue();	// 10
			int n02 = num02.intValue(); // 20
			
			int sum = n01 + n02; // Integer 래퍼 객체로 부터 반환받은 기본 데이터들의 합
			
			System.out.println("두 정수의 합 : " + sum);
			// sum 변수에 저장된 값을 2진수, 8진수, 16진수 형태의 문자열로 변환해서 반환하는 메소드
		...
		Integer num03 = new Integer(30);
			...
			// Integer 객체에 저장된 double 타입의 값으로 변환해서 리턴하는 메소드
			System.out.println(num03.doubleValue()); 	// 30.0
			
			// static int parseInt(String s ) 메소드 : 
			// 숫자 형태의 문자열을 매개변수 s로 전달받아
			// 정수로 변환해서 int 정수로 반환하는 메소드
			// 예) "100" -> 100
