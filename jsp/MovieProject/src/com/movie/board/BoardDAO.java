...
/*	final String DRIVER = "com.mysql.jdbc.Driver";
	...
	public int replyDelete(ReplyBean reBean) {
		int check = -1;
		
		
		try {
			conn = getConnection();
			sql = "select user_id from reply_list where reply_num=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, reBean.getReply_num());
			rs = pstmt.executeQuery();
			
			if(rs.next()) {
				// 검색결과 있을 때 -> 사용자 아이디가 있을 때
				if( reBean.getUser_id().equals(rs.getString("user_id"))) {
